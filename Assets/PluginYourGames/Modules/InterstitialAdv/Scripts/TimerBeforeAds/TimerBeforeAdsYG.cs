...
namespace YG
{
    public class TimerBeforeAdsYG : MonoBehaviour
    {
        ...
        private void OnEnable()
        {
                ...
                // ⚠️ 1) если реклама уже отключена – ничего не делаем
          if (SaveBridge.Saves.adsDisabled)
    {
        if (secondsPanelObject) secondsPanelObject.SetActive(false);
        enabled = false;
        return;
    }
        ...
        {
             if (!SaveBridge.Saves.adsDisabled)          // подписывались ли?
            YG2.onOpenAnyAdv -= RestartTimer;
        }
        ...
        IEnumerator CheckTimerAd()
        {
            while (true)
            {
                if (SaveBridge.Saves.adsDisabled) yield break;
