...
namespace Invector.vCharacterController.vActions
{
    ...
    public class vLadderAction : vActionListener
    {
        ...
        public float climbMoveSpeed = 2.0f;

[Tooltip("Скорость перемещения по лестнице (м/с) при ускорении")]
public float fastClimbMoveSpeed = 3.5f;

// Множители СКОРОСТИ АНИМАЦИИ (визуально), не влияют на перемещение
[Tooltip("Множитель скорости анимации при подъёме/спуске (обычный)")]
public float climbAnimSpeedMul = 1.0f;

[Tooltip("Множитель скорости анимации при подъёме/спуске (ускоренный)")]
public float fastClimbAnimSpeedMul = 1.25f;

// Отключать ли root motion на фазе карабканья (чтобы движение было полностью вручную)
[Tooltip("Отключать root motion в состояниях ClimbLadder и двигать персонажа вручную")]
public bool decoupleFromRootMotion = true;
        ...
        // Сохранённые значения для восстановления после выхода с лестницы
...
Vector3 savedLocalScale;
        ...
        void SafeParentTo(Transform newParent)
{
    if (!newParent) return;

    // Сохраняем текущего родителя и локальный масштаб
    savedParent = transform.parent;
    savedLocalScale = transform.localScale;

    // Парентимся с сохранением мировой позы
    transform.SetParent(newParent, true);

    // Компенсируем неединичный масштаб родителя
    Vector3 p = newParent.lossyScale;
    float sx = Mathf.Approximately(p.x, 0f) ? 1f : p.x;
    float sy = Mathf.Approximately(p.y, 0f) ? 1f : p.y;
    float sz = Mathf.Approximately(p.z, 0f) ? 1f : p.z;

  //  Vector3 ls = transform.localScale;
   // transform.localScale = new Vector3(ls.x / sx, ls.y / sy, ls.z / sz);
}
...
void RestoreParent()
{
    if (savedParent != null)
    {
       ...
       // Парентинг без изменения размера персонажа
if (targetLadderAction.targetCharacterParent)
{
    SafeParentTo(targetLadderAction.targetCharacterParent);
}

// Сохраняем режим root motion и при необходимости отключаем (движение будем делать вручную)
savedApplyRootMotion = tpInput.cc.animator.applyRootMotion;
if (decoupleFromRootMotion)
{
    tpInput.cc.animator.applyRootMotion = false;
}
        ...
        protected virtual void UsingLadder()
        {
            ...
            {
                tpInput.cc.animator.speed = Mathf.Lerp(tpInput.cc.animator.speed, currentClimbSpeed, 0.5f * Time.deltaTime);
            }
            else
            {
        ...
        public virtual void ResetPlayerSettings()
        {
            ...
            // Вернуть исходный режим root motion
tpInput.cc.animator.applyRootMotion = savedApplyRootMotion;

            // Вернуть родителя и локальный масштаб (чтобы не «зависнуть» под лестницей)
            RestoreParent();
 speed = 0f;
            targetLadderAction = null;
            isUsingLadder = false;
