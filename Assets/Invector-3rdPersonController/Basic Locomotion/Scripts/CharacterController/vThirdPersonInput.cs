...
namespace Invector.vCharacterController
{
    ...
    public class vThirdPersonInput : vMonoBehaviour, vIAnimatorMoveReceiver
    {
        ...
        public bool rotateOnlyWhenRMB = true;
        ...
        [Header("Mouse look")]
        [Tooltip("Требовать зажатую правую кнопку для вращения камеры")]
        public bool requireRightMouseForCamera = true;
        ...
        public virtual bool lockUpdateMoveDirection { get; set; }     
        
        public static bool ForceCursorVisible = true;   // где‑нибудь в проекте

void TryLockCursor()
{
    if (ForceCursorVisible) return;             // никогда не блокируем
    Cursor.lockState = CursorLockMode.Locked;
    Cursor.visible   = false;
}           // lock the method UpdateMoveDirection
        ...
        protected virtual void Start()
        {
            ...
            //LockCursor(unlockCursorOnStart);
                ...
                Cursor.lockState = CursorLockMode.None;     // ← добавили
    Cursor.visible   = true;  
...
public virtual void CameraInput()
{
    ...
    // ► блокируем поворот, пока не зажата правая кнопка мыши
    bool rmb = !rotateOnlyWhenRMB || Input.GetMouseButton(1);
    float Y = rmb ? rotateCameraYInput.GetAxis() : 0f;
    float X = rmb ? rotateCameraXInput.GetAxis() : 0f;
    ...
    if (!lockCameraInput && rmb)                 // зум разрешаем тоже только при RMB
        tpCamera.Zoom(cameraZoomInput.GetAxis());
}
